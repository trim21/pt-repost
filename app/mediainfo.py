...
class Extra:
    ...
    acmod: str = ""
    lfeon: str = ""
    dialnorm__average: str = Field("", alias="dialnorm_Average")
    dialnorm__minimum: str = Field("", alias="dialnorm_Minimum")
    compr__average: str = Field("", alias="compr_Average")
    compr__minimum: str = Field("", alias="compr_Minimum")
    compr__maximum: str = Field("", alias="compr_Maximum")
    compr__count: str = Field("", alias="compr_Count")
...
__t = TypeAdapter[MediaInfo](MediaInfo)
...
def parse_mediainfo_json(s: str) -> MediaInfo:
    ...
    return __t.validate_python(obj | {"video": videos, "audio": audio, "text": text})
...
def extract_mediainfo_from_file(file: Path) -> tuple[str, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="pt-repost-") as tempdir:
        ...
        run_command(command, cwd=str(file.parent))
